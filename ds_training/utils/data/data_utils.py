...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    ...
    if "Dahoas/rm-static" in dataset_name:
    ...
    elif "local/jsonfile_verify" in dataset_name:
        # chat_path = os.path.abspath(
        #     os.path.join(os.path.dirname(__file__), os.path.pardir,
        #                  os.path.pardir, os.path.pardir))
        # if not (os.path.isfile(chat_path + '/data/train_ins.json')
        ...
        #     )
        # chat_path = '/cpfs/user/chennuo/CN/gsm8k-ScRel'
        chat_path = dataset_name.split('local/jsonfile_verify')[-1]
        return raw_datasets.LocalJsonFile_verifyDataset(output_path, seed, local_rank,
                                                 dataset_name, chat_path)   
    
        
    elif "local/jsonfile_graph" in dataset_name:
      
        chat_path = dataset_name.split('local/jsonfile_graph')[-1]
        return raw_datasets.LocalJsonFile_GraphDataset(output_path, seed, local_rank,
                                                 dataset_name, chat_path)     
        ...
        for i, tmp_data in enumerate(current_dataset):
            ...
            if chosen_sentence is not None:
                ...
                chosen_token["input_ids"] = chosen_token["input_ids"].squeeze(
                    0)
                chosen_token["prompt_input_ids"] = prompt_token["input_ids"].squeeze(
                    0)
...
def create_dataset(local_rank, dataset_name, data_split, output_path,
    ...
    print('get_raw_dataset', len(train_dataset))
    ...
    print('get_raw_dataset_split_index', len(train_dataset))
