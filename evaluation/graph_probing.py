...
def main(
    ...
    tasks = ['cycle', 'connectivity', 'hamilton', 'bipartite', 'flow', 'shortest', 'triplet', 'topology']
    # tasks = ['connectivity', 'cycle',  'flow', 'substructure', 'triplet']
    # tasks = ['connectivity', 'flow']
    # tasks = ['cycle',  'triplet']
    # tasks = ['flow']
    # if args.lang_only == '':
    #     langs = test_files
    ...
    for seed in  range(1):
        ...
        for lang in tasks:
            ...
            with open(f'../datasets/probing_test/{lang}_test.json') as f:
            # with open(f'/hpc2hdd/home/yli258/jhaidata/Graph-Reasoning-LLM/datasets/train_set_v3/{lang}_train.json') as f:
                datas = f.readlines()
            ...
            # probing
            with open('../datasets/demos/2shot/' + lang + '.txt', 'r', encoding='utf-8') as file:
                ICL = file.read()
            for i in range(len(gsm8k_datas)):
                question_index = gsm8k_datas[i]["input_prompt"].find("Q: ")
                question = gsm8k_datas[i]["input_prompt"][question_index:]
                gsm8k_datas[i]["input_prompt"] = ICL + " \n" + question
...
def gsm8k_batch_gen(
    ...
    # probing
    input_str_list = curs_gsm8k_questions
    ...
    def batch_llama(input_strs):
        ...
        output_ids = model.generate(
            ...
            generation_config=GenerationConfig(
                ...
                do_sample=False,
                temperature=0,  # t=0.0 raise error if do_sample=True
            ),
        ).tolist()
