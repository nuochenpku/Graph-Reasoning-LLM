...
def main():
    ...
    batch_num = 20
    G_list, Q_list, std_list, n1_list, edges = [], [], [], [], []
    for i in tqdm(range((g_num + batch_num - 1) // batch_num)):
        
        for j in range(i*batch_num, min(g_num, (i+1)*batch_num)):
            with open("/cpfs/user/chennuo/CN/NLGraph/NLGraph/matching/graph/"+args.mode+"/full/graph"+str(j)+".txt","r") as f:
    ...
    print(len(Q_list))
    print(len(G_list))
    with open('/cpfs/user/chennuo/CN/NLGraph/NLGraph/matching/matching_full.json', 'a') as w:
        for Q, G, E, n1, std in zip(Q_list, G_list, edges, n1_list, std_list):
